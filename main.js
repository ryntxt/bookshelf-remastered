document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateBookshelf = (listOfBook) => {
    ...
    listOfBook.forEach((book) => {
      book.isComplete ? completeBookshelfList.append(createArticleBook(book)) : incompleteBookshelfList.append(createArticleBook(book));
    });
  };
  ...
  const handleSearch = (e) => {
    ...
    const searchResults = books.filter((book) => book.title.toLowerCase().includes(query) || book.author.toLowerCase().includes(query) || book.year.toString().includes(query));

    if (searchResults.lenght !== 0) updateBookshelf(searchResults);
  };
  ...
  document.getElementById("inputBook").addEventListener("submit", (e) => {
    ...
    if (isDuplicateBook(bookTitle)) alert(`Buku dengan judul ${bookTitle} sudah ada dalam daftar.`);
    else {
      const book = {
        id: new Date().getTime(),
